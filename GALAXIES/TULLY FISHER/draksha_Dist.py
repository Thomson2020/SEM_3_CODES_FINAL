...
# Print the distance for each galaxy
print("Galaxy Distances (in Mpc):")
for index, row in spiral_galaxies.iterrows():
    print(f"Galaxy {index + 1}: Distance = {row['distance']:.2f} Mpc")

# Save the distances to a CSV file
spiral_galaxies[['distance']].to_csv('dist.csv', index=False)
...
# Print the distance for each galaxy again (optional)
print("Galaxy Distances (in Mpc):")
for index, row in spiral_galaxies.iterrows():
    print(f"Galaxy {index + 1}: Distance = {row['distance']:.2f} Mpc")
